...
function Deal(type: string, val: any) {
    const TYPE_LIMIT: any = {
        h: [0, 360],
        s: [0, 100],
        l: [0, 100],
        v: [0, 100],
    ...
    val = val.toString().includes('%') ? (val.toString().replace('%', '') / 100) * TYPE_LIMIT[type][1] : val
...
export default function colorConvert(value: string) {
    ...
    if (REG.hsl.test(value)) {
        // hsl convert to hex
        const c: any = REG.hsl.exec(value)
        const h = Deal('h', c[1])
        const s = Deal('s', c[2])
        const l = Deal('l', c[3])
        ...
        const c: any = REG.hsla.exec(value)
        const h = Deal('h', c[1])
        const s = Deal('s', c[2])
        const l = Deal('l', c[3])
        const a = Deal('a', c[4])
        ...
        // hsv convert to hex
        const c: any = REG.hsv.exec(value)
        const h = Deal('h', c[1])
        const s = Deal('s', c[2])
        const v = Deal('v', c[3])
        ...
        const c: any = REG.hsva.exec(value)
        const h = Deal('h', c[1])
        const s = Deal('s', c[2])
        const v = Deal('v', c[3])
        const a = Deal('a', c[4])
        ...
        // rgb convert to hex
        const c: any = REG.rgb.exec(value)
        r = Deal('r', c[1])
        g = Deal('g', c[2])
        b = Deal('b', c[3])
    } else if (REG.rgba.test(value)) {
        // rgba
        const c: any = REG.rgba.exec(value)
        r = Deal('r', c[1])
        g = Deal('g', c[2])
        b = Deal('b', c[3])
        alpha = Deal('a', c[4])
